        ...
        <center>
            <form action="editpatient.php?id=<?php echo $id; ?>" method="POST">
                ...
                <select name="bloodgroup" class="form" required="required">
                    ...
                    <option value="A" <?=($row['bloodgroup'] == 'A' ? 'selected' : '')?>>A</option>
                    <option value="B" <?=($row['bloodgroup'] == 'B' ? 'selected' : '')?>>B</option>
                    <option value="AB" <?=($row['bloodgroup'] == 'AB' ? 'selected' : '')?>>AB</option>
                    <option value="o" <?=($row['bloodgroup'] == 'o' ? 'selected' : '')?>>o</option>
                </select><br><br>
                <select name="birthyear" class="form" required="required">
                    ...
                    <?php foreach (range(1970, date('Y')) as $year): ?>
                        <option value="<?= $year ?>" <?=($row['birthyear'] == $year ? 'selected' : '')?>><?= $year ?></option>
                    <?php endforeach; ?>
                </select><br><br>
